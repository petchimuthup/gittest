...
class AddUpdate extends \Magento\Framework\App\Action\Action
{
    /**
    ...
    public function execute()
    {
        ...
        $customer_check = $customer->setWebsiteId($websiteId)->load($customerId);
        ...
        if (!$customer_id) 
        {
            ...
            try {
                ...
                $updateCustomer = $this->customerRepository->getById($getId);

                $companyCustomerAttributes = $updateCustomer->getExtensionAttributes()->getCompanyAttributes();

                if ($companyCustomerAttributes) {
                    
                    $companyCustomerAttributes->setStatus($userStatus);
                }

                $this->customerRepository->save($updateCustomer);
                $this->messageManager->addSuccessMessage(__('User created successfully!.'));
        ...
        return $resultRedirect->setPath('company/store/user/');
