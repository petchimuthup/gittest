...
namespace ISN\PaymentAccount\Test\Unit\Model\ResourceModel;
...
class CreditLimitTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testChangeBalance()
    {
        ...
        $rowData = [
            \ISN\PaymentAccount\Api\Data\CreditLimitInterface::BALANCE => 25,
            \ISN\PaymentAccount\Api\Data\CreditLimitInterface::CURRENCY_CODE => 'EUR',
        ];
        ...
        $operationCurrency->expects($this->once())->method('convert')->with(
            $value,
            $rowData[\ISN\PaymentAccount\Api\Data\CreditLimitInterface::CURRENCY_CODE]
        )->willReturn($convertedValue);
        $operationCurrency->expects($this->once())->method('getRate')
            ->with($rowData[\ISN\PaymentAccount\Api\Data\CreditLimitInterface::CURRENCY_CODE])
            ->willReturn(0.7);
        $this->connection->expects($this->once())->method('update')->with(
            null,
            [\ISN\PaymentAccount\Api\Data\CreditLimitInterface::BALANCE => 37],
            $condition
        );
