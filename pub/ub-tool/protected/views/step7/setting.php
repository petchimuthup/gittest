                    ...
                    <?php if ($salesObjects): ?>
                        <ul class="list-group ver-nav">
                            <?php foreach ($salesObjects as $object => $info): ?>
                                <?php
                                /*$checked = ((!in_array($object , array('sales_aggregated_data', 'active_quote')) AND !sizeof($selectedSalesObjects))
                                    OR in_array($object, $selectedSalesObjects)) ? 'checked' : '';*/
                                $checked = in_array($object, $selectedSalesObjects) ? 'checked' : '';
                                /*$disabled = (!in_array($object , array('sales_aggregated_data', 'active_quote'))
                                    || ($checked && in_array($object, $migratedSalesObjects)))
                                    ? 'class="read-only" readonly="readonly" onclick="event.preventDefault();"'
                                    : '';*/
                                $disabled = (($checked && in_array($object, $migratedSalesObjects)))
                                    ? 'class="read-only" readonly="readonly" onclick="event.preventDefault();"'
                                    : '';
