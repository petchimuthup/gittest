...
class Mage2SalesQuote extends Mage2ActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('store_id, is_active, is_virtual, is_multi_shipping, customer_note_notify, customer_is_guest,
			trigger_recollect, is_persistent, gift_message_id', 'numerical', 'integerOnly'=>true),
			array('items_count, orig_order_id, customer_id, customer_tax_class_id, customer_group_id, is_changed', 'length', 'max'=>10),
			array('items_qty, store_to_base_rate, store_to_quote_rate, base_subtotal_with_discount', 'length', 'max'=>12),
			array('base_currency_code, store_currency_code, quote_currency_code, checkout_method, customer_email,
			customer_firstname, customer_lastname, customer_note, applied_rule_ids, password_hash, coupon_code,
			global_currency_code, customer_taxvat, customer_gender', 'length', 'max'=>255),
			...
			array('subtotal_with_discount, subtotal, base_subtotal, base_to_global_rate, base_to_quote_rate, grand_total,
			 base_grand_total', 'length', 'max'=>20),
		);
	}
