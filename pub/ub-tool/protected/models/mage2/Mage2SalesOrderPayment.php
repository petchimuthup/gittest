...
class Mage2SalesOrderPayment extends Mage2ActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('cc_exp_month, cc_ss_start_year', 'length', 'max'=>12),
			// rules applied from M2.3.1: changed max length allowed from 12 to 20
			array('base_shipping_captured, shipping_captured, amount_refunded, base_amount_paid, amount_canceled,
			...
			base_amount_refunded, amount_ordered, base_amount_canceled', 'length', 'max'=>20),
			array('echeck_bank_name, method, cc_ss_start_month, cc_owner, cc_number_enc', 'length', 'max'=>128),
			array('cc_debug_request_body, cc_secure_verify, protection_eligibility, cc_approval, cc_status_description,
			echeck_type, cc_debug_response_serialized, cc_cid_status, cc_type, po_number, echeck_routing_number,
			account_status, anet_trans_method, cc_debug_response_body, cc_ss_issue, echeck_account_name, cc_avs_status,
			cc_trans_id, address_status', 'length', 'max'=>32),
			array('cc_last_4', 'length', 'max'=>100),
			array('echeck_account_type, last_trans_id', 'length', 'max'=>255),
