...
class UBMigrate extends CActiveRecord
{
    ...
    const RUN_MODE_CONTINUE = 'run';
    const RUN_MODE_DELTA = 'delta';
    ...
    public static function generateCategoryTreeHtml($categoryTree, $settingData, $level = 1)
    {
        $selectedAll = (isset($settingData['select_all_category'])) ? $settingData['select_all_category']  : false;
        ...
        foreach ($categoryTree as $category) {
            //check has selected
            $checked = ($selectedAll || in_array($category->entity_id, $selectedCategoryIds)) ? true : false;
            //check has migrated
            ...
            if ($category->children) {
                $level++;
                $html .= self::generateCategoryTreeHtml($category->children, $settingData, $level);
            }
            $html .= '</li>';
        }
        $html .= '</ul>';
    ...
    public static function getTotalSalesChildObject($objectId, $strStoreIds, $strCustomerIds)
    {
        ...
        switch ($objectId) {
            ...
            case 'quote':
                ...
                break;
            case 'active_quote':
                $sql = "SELECT COUNT(*) FROM `{$tablePrefix}sales_flat_quote` e WHERE (e.is_active = 1 AND e.store_id IN ({$strStoreIds})) AND e.entity_id NOT IN (SELECT quote_id FROM `{$tablePrefix}sales_flat_order`)";
                $total = Yii::app()->db1->createCommand($sql)->queryScalar();
                break;
            case 'payment':
    ...
    public static function updateCurrentOffset($entityName, $offset, $classEndfix = '', $isLatest = false)
    {
    ...
    public static function resetOffset($stepIndex)
    {
        $tablePrefix = Yii::app()->db->tablePrefix;
        $tableName = "{$tablePrefix}ub_migrate_map_step_{$stepIndex}";
        $query = "UPDATE `{$tableName}` SET `offset` = '0'";
        $result = Yii::app()->db->createCommand($query)->execute();
        return $result;
    }

    public static function getLastMigrationTime($tableSuffix, $entityName = null)
    {
        $result = null;
        if (isset($tableSuffix)) {
            $tablePrefix = Yii::app()->db->tablePrefix;
            $tableName = "{$tablePrefix}ub_migrate_map_step_{$tableSuffix}";
            $query = "SELECT MAX(created_time) FROM {$tableName}";
            if ($entityName) {
                $entityName = str_replace(['{{', '}}'], ['', ''], $entityName);
                $query .= " WHERE entity_name = '{$entityName}'";
            }
            $lastMigrationTime = Yii::app()->db->createCommand($query)->queryScalar();
            if ($lastMigrationTime) {
                $lastMigrationTime = explode(" ", $lastMigrationTime);
                $result = $lastMigrationTime[0] . " 00:00:00";
            }
        }
