            ...
            try {
                if ($selectedObjects) {
                    ...
                    if (in_array('review', $selectedObjects)) {
                         ...
                         */
                        $condition = '';
                        $max1 = Mage1Review::model()->count($condition);
                        $offset1 = UBMigrate::getCurrentOffset(8, Mage1Review::model()->tableName());
                        //get list objects by limit and offset
                        $list1 = UBMigrate::getListObjects('Mage1Review', $condition, $offset1, $this->limit, 'review_id ASC');
                        if ($list1) {
                ...
                if ($this->errors) {
                    ...
                    if ($offset1 >= $max1 AND $offset2 >= $max2 AND $offset3 >= $max3
                        ...
                        if ($step->updateStatus(UBMigrate::STATUS_FINISHED)) {
                            ...
                            UBMigrate::updateSetting($this->stepIndex, 'migrated_objects', $selectedObjects);
                            UBMigrate::updateSetting($this->stepIndex, 'migrated_child_objects', $selectedChildObjects);
