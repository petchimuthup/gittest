        ...
        if ($check['allowed']) {
            ...
            try {
                ...
                if ($selectedProductTypes) {
                    ...
                    $lastRecordOffset = Mage1CatalogProductEntity::model()->count($condition);
                    ...
                    */
                    ...
                    if ($offset >= $max) { //if has migrated all products
                        ...
                        if ($this->strDeltaProductIds) {
                            $condition .= " AND (product_id IN ({$this->strDeltaProductIds}) OR linked_product_id IN ({$this->strDeltaProductIds}))";
                        }
                        ...
                        if (in_array('configurable', $selectedProductTypes)) {
                            ...
                            if ($this->strDeltaProductIds) {
                                $condition = "(product_id IN ({$this->strDeltaProductIds}) OR parent_id IN ({$this->strDeltaProductIds}))";
                            }
                            //get max total
                        ...
                        if ( in_array('grouped', $selectedProductTypes)
                            ...
                            $condition = '';
                            $lastRecordOffset4 = Mage1CatalogProductRelation::model()->count($condition);
                            if ($this->strDeltaProductIds) {
                                $condition = "parent_id IN ({$this->strDeltaProductIds}) OR child_id IN ({$this->strDeltaProductIds})";
                            }
                ...
                if ($this->errors) {
                    ...
                    if ($offset >= $max AND $offset1 >= $max1 AND $offset2 >= $max2
                        ...
                        if ($step->updateStatus(UBMigrate::STATUS_FINISHED)) {
                            ...
                            UBMigrate::updateCurrentOffset(Mage1CatalogProductEntity::model()->tableName(), $lastRecordOffset, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1CatalogProductLink::model()->tableName(), $lastRecordOffset1, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1CatalogProductSuperLink::model()->tableName(), $lastRecordOffset2, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1CatalogProductSuperAttribute::model()->tableName(), $lastRecordOffset3, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1CatalogProductRelation::model()->tableName(), $lastRecordOffset4, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1CatalogProductBundleOption::model()->tableName(), $lastRecordOffset5, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1DownloadableLink::model()->tableName(), $lastRecordOffset6, $this->stepIndex, true);
                            UBMigrate::updateCurrentOffset(Mage1DownloadableSample::model()->tableName(), $lastRecordOffset7, $this->stepIndex, true);
    ...
    private function _migrateCatalogProducts($products, $mappingWebsites, $mappingStores, $keepOriginalId)
    {
        ...
        foreach ($products as $product) {
            ...
            if (is_null($productId2)) {
                ...
                if ($found) {
                    $sku2 =  "{$product->entity_id}_{$product->sku}";
                }
            ...
            if ($product2->entity_id) {
                ...
                $this->_migrateCatalogProductStockItem($product->entity_id, $product2->entity_id, $product2->sku, $keepOriginalId);
        ...
        if ($stockItems) {
            foreach ($stockItems as $stockItem) {
                ...
                if (!$stockItem2->save()) {
                     ...
                     */
                    $sku = addslashes($sku);
                    $condition = "source_code = 'default' AND sku = '{$sku}'";
