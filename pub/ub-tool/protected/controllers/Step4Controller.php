        ...
        if ($result['allowed']) {
            ...
            if (Yii::app()->request->isPostRequest) {
                ...
                if ($selectedCategoryIds) {
                    ...
                    $settingData = [
                        'category_ids' => (!$selectAll) ? $selectedCategoryIds : [],
        ...
        if ($check['allowed']) {
            ...
            try {
                ...
                if ($selectedCategoryIds) {
                    ...
                    $lastRecordOffset = Mage1CatalogCategoryEntity::model()->count($condition);
                    ...
                    if ($this->runMode == UBMigrate::RUN_MODE_DELTA) {
                        $lastMigrationTime = UBMigrate::getLastMigrationTime($this->stepIndex, 'catalog_category_entity');
                ...
                if ($this->errors) {
                    ...
                    if ($offset >= $max) {
                        ...
                        if ($step->updateStatus(UBMigrate::STATUS_FINISHED)) {
                            //update offset to last record
                            UBMigrate::updateCurrentOffset(Mage1CatalogCategoryEntity::model()->tableName(), $lastRecordOffset, $this->stepIndex, true);
                        ...
                        $rs['message'] = Yii::t(
                            'frontend',
                            "{$breakLine}{$msg} %s1 Catalog Categories",
